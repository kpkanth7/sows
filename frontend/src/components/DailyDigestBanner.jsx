...
export default function DailyDigestBanner() {
  ...
  if (loading) {
    return <div className="digest-banner digest-banner-loading"><div className="skeleton skeleton-title" /><div className="skeleton skeleton-text" /></div>;
  }
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <button
          className="theme-toggle digest-banner-toggle"
          onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div className="mt-2">
          <p className="text-sm m-0 digest-banner-summary">{digest.summary}</p>
          {tickers.length > 0 && (
            <div className="flex gap-2 flex-wrap mt-3">
              {tickers.map(t => (
                <span key={t} className="badge badge-gold digest-banner-chip">{t}</span>
              ))}
            </div>
          )}
        </div>
